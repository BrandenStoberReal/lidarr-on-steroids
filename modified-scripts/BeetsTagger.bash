...
if [ "$enableBeetsTagging" != "true" ]; then
  log "Beets tagging is disabled, please enable by setting \"enableBeetsTagging=true\" in \"/scripts-config/extended.conf\""
  exit 0
fi
...
ProcessWithBeets () {
	...
	if [ -f /scripts-config/extended/library-lidarr.blb ]; then
		rm /scripts-config/extended/library-lidarr.blb
		sleep 0.5
	fi
	if [ -f /scripts-config/extended/extended/beets-lidarr.log ]; then 
		rm /scripts-config/extended/extended/beets-lidarr.log
		sleep 0.5
	fi

	if [ -f "/scripts-config/extended/beets-lidarr-match" ]; then 
		rm "/scripts-config/extended/beets-lidarr-match"
		sleep 0.5
	fi
	touch "/scripts-config/extended/beets-lidarr-match"
	sleep 0.5

        log "$1 :: Begin matching with beets!"
	beet -c /scripts-config/extended/beets-config-lidarr.yaml -l /scripts-config/extended/library-lidarr.blb -d "$1" import -qC "$1" 2>&1 | tee -a "/scripts-config/logs/$logFileName"
	# Fix tags
	log "$1 :: Fixing Tags..."
	...
	if [ -f /scripts-config/extended/logs/beets.log ]; then 
		rm /scripts-config/extended/logs/beets.log
		sleep 0.5
	fi
